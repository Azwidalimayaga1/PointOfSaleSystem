...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name) { $error = 'Name is required.'; }
        ...
        try {
            if ($id) {
                $db->prepare("UPDATE customers SET name=?, phone=?, email=?, address=?, notes=? WHERE id=?")->execute([$name, $phone, $email, $address, $notes, $id]);
            } else {
...
<div class="form-group"><label>Address</label><textarea name="address" class="form-control" rows="2"><?= e($customer['address'] ?? '') ?></textarea></div>
<div class="form-group"><label>Notes</label><textarea name="notes" class="form-control" rows="2"><?= e($customer['notes'] ?? '') ?></textarea></div>
